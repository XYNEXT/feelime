...
- 底部横向分类条（两侧渐隐 mask + 相邻类目露头作滑动暗示）：
  ...
  `中`/`En` 标识当前表（借模式切换键的双写语法），切换输入模式后回到
  跟随模式，「最近」回填跟随同一张表。
- **方向分类**：不发文本，格子上屏字形、点击发宿主按键事件（与控制键层
  同一 keycode 白名单通道）：←↑↓→ / Home End PgUp PgDn / Tab Esc Del
  空格 换行 + ⌫；全部支持按住连发；不记入「最近」。
...
| `feelime_keyboard` | 键盘热更新状态（active/previous 指针、更新源，§8） |
...
设置面板是键盘内流内区块（打开时替换按键层，观感是「换了一页」），行集：
色彩模式 / 滑动跟手 / 双拼键位 / 快捷切换 / 长按菜单 / 键盘高度。二级
内容（键位图、快捷对勾选、长按菜单勾选+排序）就地展开或子页。
...
- 结构：首页（输入法状态 + 分组入口 + 输入测试入口）→ 二级页：键盘与
  输入（双拼键位图/定制键盘）、语音识别、键盘热更新、关于（版本信息
...
组合修饰位，§11）；上滑副符号、下滑大写；长按字母弹 alt 弹层（拖远
取消、拖回选择）。
...
- 长按弹层：按「距最近浮层格心 ≤60px」判定选中（屏幕右缘键的浮层被
  clamp 后，合法目标格离按键可达 100px+，按绝对距离会误杀）；离所有格
  60px 外进入取消态（渐隐可恢复），170px 全隐；`elementFromPoint` 出键
  取消长按；长按 timer 挂 touchcancel 清理（来电/手势中断不派发
  touchend，浮层不得照弹）。
...
- 入口：长按空格（「松手结束」）；短按工具栏麦克风（「点击任意位置
  结束」，点浮层内外都结束并提交）；浮层右上 ✕ 只**取消**本次语音。
