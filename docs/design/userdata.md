...
原 ✕ 按钮单击即整段丢弃听写，长篇听写误触挫败感强。改为：

- 按钮从「×」改为「↺ 撤销」胶囊（图标+文字），语义从"关闭"改为
  明确的"弃稿"；aria-label 撤销本次听写。
- **两击确认**：partialText 已有内容时，第一击只进入武装态
  （按钮变红、文案「再点一次撤销」，2.6s 无第二击自动复原），
  第二击才 cancelVoice；还没说话时单击直接撤销。
- 撤销成功后 toast「已撤销本次听写」；会话结束（onNativeState 离开
  listening/loading/stopping）自动清除武装态。
